...
import { graphql, inlineImports } from '@firsthandjs/data/vite';
import { plugin } from '@firsthandjs/data/codegen';
import { parseGraphQL } from '@firsthandjs/data';
...
describe('the codegen plugin', () => {
  ...
  it('declares a typed module per operation file', () => {
    ...
    expect(out).toContain("import('@firsthandjs/data').GraphQLDocument<");
  ...
  it('takes the types path from the config', () => {
    const out = plugin(null, [document('/src/notes.gql', 'Notes', 'query')], {
      ...
      documentTypePath: '@firsthandjs/data',
    });
  ...
  it('skips a file with no operation, and one with no location', () => {
    ...
    // would be a mistake.
    expect(out).toContain('Generated by @firsthandjs/data/codegen');
  });
