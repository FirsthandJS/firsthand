...
site costs 1.02x the single-shape site — within noise — and the control costs
28.4x, which is how the measurement shows it could have detected a problem.
...
| R6  | 6 kB gzip target unreachable with the full feature set                                | A headline goal is missed                                | `npm run build`             | **closed**: the build fails above the budget in `scripts/build.mjs`, which is 7.50 kB gzip with everything imported — raised from 6 kB when render functions and server rendering landed, each time with the number in the commit that raised it. Routing and the data layer are separate packages, outside that budget. |
...
| R8  | Compiler cannot always prove props destructuring safe                                 | Either silent stale props or false-positive build errors | Compiler test corpus        | **closed**: destructuring is rewritten into live reads, and the cases the rewrite cannot prove safe — an assignment to a destructured prop, a computed key, a rest element that escapes — are build errors naming the property (ADR-0005, reversed after use).                                                           |
